...
void reserve_room(shared_data_t *shared_data, int room_type, int server_fd, struct sockaddr_in client) {
    int i, room_num;
    ...
    sendto(server_fd, response, strlen(response) + 1, 0, (struct sockaddr *) &client, sizeof(client));
}
...
int main(int argc, char *argv[]) {
    ...
    while (1) {    
        ...
        // Read client request
        unsigned int addrlen_client = sizeof(client);
        valread = recvfrom(server_fd, buffer, 1024, 0, (struct sockaddr *) &client, &addrlen_client);
        ...
        if (room_type == 0) {
            ...
            sendto(server_fd, response, strlen(response) + 1, 0, (struct sockaddr *) &client, sizeof(client));
            char result[5];
            valread = recvfrom(server_fd, result, 5, 0, (struct sockaddr *) &client, &addrlen_client);
            room_type = atoi(result);
            reserve_room(&shared_data, room_type, server_fd, client);
        }
    }
